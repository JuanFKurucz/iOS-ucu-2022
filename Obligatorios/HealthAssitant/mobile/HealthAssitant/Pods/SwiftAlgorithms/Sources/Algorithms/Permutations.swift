...
extension PermutationsSequence: Sequence {
    ...
    public struct Iterator: IteratorProtocol {
        ...
        internal init(_ permutations: PermutationsSequence) {
            base = permutations.base
            baseCount = permutations.baseCount
            kRange = permutations.kRange
            indexes = Array(permutations.base.indices)
        }
        ...
        internal mutating func nextState() -> Bool {
            let countToChoose = kRange.lowerBound
...
public extension Collection {
    ...
    @inlinable
    func permutations<R: RangeExpression>(
        ofCount kRange: R
...
extension UniquePermutationsSequence where Base.Element: Hashable {
    @inlinable
    static func _indexes(_ base: Base) -> [Base.Index] {
    ...
    init(_ elements: Base) {
        indexes = Self._indexes(elements)
        base = elements
        kRange = indexes.count ..< (indexes.count + 1)
    }

    @inlinable
    init<R: RangeExpression>(_ base: Base, _ range: R)
        where R.Bound == Int
    {
        indexes = Self._indexes(base)
        self.base = base

        let upperBound = indexes.count + 1
        kRange = range.relative(to: 0 ..< .max)
            .clamped(to: 0 ..< upperBound)
    }
}
...
public extension Collection where Element: Hashable {
    ...
    ///   collection.
    @inlinable
    func uniquePermutations(ofCount k: Int? = nil)
    ...
    ///   collection.
    @inlinable
    func uniquePermutations<R: RangeExpression>(
        ofCount kRange: R
